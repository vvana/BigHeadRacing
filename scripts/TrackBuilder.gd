...
# Космос: полотно — парящая платформа. Земля (звёздное поле) лежит далеко
# внизу и опорой НЕ служит: перелетевшая ограждение машина падает в пустоту
# (жалоба 03.09 «вылетаешь за трассу в космосе — машина не падает»: раньше
# за бортом лежала чёрная в звёздах земля со склоном 0.35 м/м, и машина
# ехала по ней как по обочине). Глубина такая, чтобы автовозврат
# (Main.FALL_DEPTH = 8 м) сработал задолго до дна, а плоскость всё ещё
# закрывала кадр ортокамеры: картинка плоскости сдвигается на
# 30·cos 32° ≈ 25 м при запасе земли за габаритом трассы ≥ 78 м.
const SPACE_VOID_DROP := 30.0
# Юбка ограждений в космосе: борт платформы уходит вниз на 5 м — иначе
# под тонкой кромкой полотна сразу зияет пустота.
const SPACE_SKIRT := 5.0
...
func _ready() -> void:
	...
	if kind == KIND_SAND:
		# Отбойники ЕСТЬ (просьба 14.09 «на трассе пустыня нужно установить
		# отбойники»). До того пустыня была единственной трассой без
		# ограждений: съезд на песок был легален и наказывался самим песком
		# (Car._on_sand, потолок скорости) — этот код в Car оставлен, но за
		# отбойник машина больше не попадает. Обочина по-прежнему почти
		# вровень с полотном (0.05): песок за отбойником — ровная пустыня,
		# а не ров, как у классики; юбка стены уходит в песок.
		_ground_drop = 0.05
	_build_curve()
	_sample_frames()
	_build_ground()
...
## спуск по ~69 м (пологие — на подъёме уклон ≤ 11%), между ними короткий
## гребень. 08.09 переходы растянуты с 42 до 52 м: на гребне вертикальное
## ускорение (то, чем горка «подбрасывает») падает с 8.9 до 5.8 м/с² на
## 25 м/с — машина держится дороги, а не срывается в полёт на каждом
## круге («перепад высот сделан слишком неравномерно»).
## 11.09 переходы растянуты ещё раз, 52 -> 69 м: жалоба «на подъёме машина
## немного уходит под текстуру». Замер (стенд DbgUphill, ИИ на 33 м/с) — на
## ПОДНОЖИИ дорога поднималась быстрее, чем кузов успевал за ней: подвеска
## доседала до 0.81 хода, днище касалось полотна (клиренс −0.009), низ
## колеса уходил под асфальт на 0.112 м, а через полкадра сжатая пружина
## выбрасывала машину вверх на 7 м/с. Вертикальное ускорение на подножии
## падает как квадрат длины перехода: (52/69)² — примерно вдвое. Переходы шире самой прямой и захватывают края соседних дуг —
...
## ПЕСЧАНАЯ трасса (kind == KIND_SAND): плоская пустыня, вдоль полотна
## стальные отбойники (с 14.09; до того — без ограждений).
...
## ограждения ЕСТЬ, за ними — ПУСТОТА: опоры у кромки нет, звёздное поле
## лежит на SPACE_VOID_DROP ниже (см. _ground_height), перелетевшая борт
## машина ПАДАЕТ и возвращается автовозвратом (Main._check_recovery,
## FALL_DEPTH). По верху стен светящиеся полосы (см. _build_neon_strips,
## у космоса свои цвета).
...
func _build_curve() -> void:
	...
	for i in points.size():
		...
		var t := _tangent_at(points, dirs, i) * (TURTLE_STEP / 3.0)
		_curve.set_point_in(i, -t)
		_curve.set_point_out(i, t)
	# Curve3D.closed появился только в 4.4 — замыкаем дублем первой точки.
	var t0 := _tangent_at(points, dirs, 0) * (TURTLE_STEP / 3.0)
	_curve.add_point(points[0] - center)
	_curve.set_point_in(_curve.point_count - 1, -t0)
	_curve.set_point_out(_curve.point_count - 1, t0)


## Направление касательной в опорной точке: в плане — строго по ходу
## (dirs[i], от этого зависит точность прямых и дуг), по ВЫСОТЕ — по
## уклону профиля между соседними точками.
##
## ПОЧЕМУ ТАК (08.09). Раньше касательная была строго горизонтальной,
## а высоты у соседних точек разные — значит на СКЛОНЕ каждый кубический
## сегмент выходил из точки горизонтально, разгонялся по высоте и приходил
## в следующую снова горизонтально. Профиль превращался в лесенку S-образных
## ступенек с шагом TURTLE_STEP (3 м): уклон гулял от 0 до полутора средних
## и обратно ~8 раз в секунду на полном ходу — «машина будто едет по
## стиральной доске» (жалоба 08.09). С вертикальной составляющей
## касательной профиль становится гладким (C¹), и подъём чувствуется
## одним ровным склоном.
static func _tangent_at(points: Array[Vector3], dirs: Array[Vector3],
		i: int) -> Vector3:
	var n := points.size()
	...
	# dirs[i] единичный и горизонтальный: прибавка по Y не меняет длину
	# горизонтальной проекции ручки — форма трассы в плане прежняя.
	var slope := d.y / horiz if horiz > 1.0e-4 else 0.0
	return dirs[i] + Vector3.UP * slope
...
func _ground_height(x: float, z: float) -> float:
	if kind == KIND_SPACE:
		# Пустота: ровное звёздное поле глубоко под платформой — и под
		# полотном тоже (провалившаяся сквозь тримеш машина падает и
		# возвращается, а не едет под дорогой). См. SPACE_VOID_DROP.
		return -SPACE_VOID_DROP
	var p := Vector3(x, 0, z)
...
func _build_walls() -> void:
	...
	# полосы поверху добавляет _build_neon_strips); пустыня — оцинкованный
	# дорожный отбойник: светлая сталь с металлическим бликом.
	match kind:
		KIND_SAND:
			mat.albedo_color = Color(0.68, 0.68, 0.64)
			mat.metallic = 0.55
			mat.roughness = 0.45
		KIND_NEON:
