...
func net_make_puppet() -> void:
	...
	freeze = true
	# С 27.08 марионетка снова ТВЁРДАЯ для решателя: исключения, введённые
	# 26.08 против «дикого импульса от прыгнувшей марионетки», обернулись
	# жалобой хуже — «столкновений нет, проезжаю сквозь машины» (аркадный
	# толчок _bounce_off_cars геометрию не заменяет: кузов проходил насквозь,
	# стенд TestPuppetSolid показывал проезд на 12 м за центр соперника).
	# От дикого импульса защищает кап депенетрации в _physics_process (5 м/с
	# за кадр, смотрит ТЕКУЩИЕ контакты), а корень тех рывков — фризы клиента
	# на постройке моделей — вылечен кэшем паков ещё 26.08.
