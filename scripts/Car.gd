...
## Сервер: ПОЛПУТИ от владельца этой машины до сервера, с (Net.half_rtt по
## его peer_id; у ботов 0). Стрелявший видел цель ещё и на эту величину
## позже, чем цель себя, — сервер добавляет её к отмотке (см. aim_lag).
var net_wire_lag := 0.0
...
# Сглаженные скорость и темп разворота для УПРЕЖДЕНИЯ картинки
...
var _wall_touch := false        # то же с ГИСТЕРЕЗИСОМ (см. _wall_slide)
...
func _bounce_off_cars() -> void:
	...
	for other in partners:
		...
		if closing > 0.5 and not shoved_already:
			...
			# Мультяшных звёзд при таране больше нет (просьба 31.08: «искры
			# должны быть в виде искр, а не звёздочек») — крепкий удар просто
			# даёт сноп погуще (power и так растёт со скоростью сближения).
...
func net_shot_lag() -> float:
	...
	# ЭТО ТОЛЬКО ЕГО ПОЛОВИНА (буфер + дорога от сервера к нему за вычетом
	# упреждения, протокол 19). Полпути ВЛАДЕЛЬЦА ЦЕЛИ до сервера
	# добавляет aim_lag — без него отмотка была короче правды, и выстрел
	# «перед соперником» всё равно попадал.
...
## том что место HUD считает по своей картине (Main._place_of). Лечим не
## HUD, а картинку: сдвигаем марионетку ВПЕРЁД по её же записи —
## по скорости и скорости поворота из снимков (модель постоянного
## разворота, то есть по дуге, а не по хорде: в повороте прямая
## экстраполяция уводит в стену).
##
## Компенсируем НЕ ВСЁ: доля LEAD_SHARE от буфера (остаток — честный
## запас на дыры канала) плюс свои полпути до сервера (это чистая
## задержка, гадать нечего). Потолок LEAD_MAX: чем дальше заглядываем,
## тем сильнее промах на торможении и ударе (соперник «резинит»).
## Скорость для упреждения берём СГЛАЖЕННОЙ — иначе скачки скорости в
## снимках умножались бы на упреждение и картинка задрожала бы (ровно на
## это наступили с константой LEAD выше).
##
## Доля буфера, которую компенсируем. 0.75 подобрана стендом
## TestViewLead: отставание картинки 6.9 -> 2.8 м на канале 0.25 c, а
## дальше растёт цена ошибки предсказания (соперник резко тормозит или
## врезается — картинка на мгновение убегает вперёд и ждёт, пока правда
## догонит).
const VIEW_LEAD_SHARE := 0.75
## Потолок упреждения: дальше цена ошибки предсказания перевешивает.
const VIEW_LEAD_MAX := 0.25
## Текущее упреждение (с), общее на все марионетки — как и буфер.
static var net_view_lead := 0.0
...
static func net_note_gap(gap: float) -> void:
	...
	_buf_n += 1
	# Упреждение ползёт к цели плавно (снимков ~60/с, доля 0.05 — около
	# трети секунды на переход): скачок упреждения — это скачок картинки.
	var want := clampf(net_buf_delay * VIEW_LEAD_SHARE + Net.half_rtt(1),
			0.0, VIEW_LEAD_MAX)
	net_view_lead = lerpf(net_view_lead, want, 0.05)


## Сколько картинка соперников У МЕНЯ отстаёт от их «сейчас» ПОСЛЕ
## упреждения: буфер + дорога от сервера ко мне − упреждение. Едет серверу
## 13-м числом состояния (протокол 19), и по ней он отматывает цели моих
## выстрелов. Полпути ВЛАДЕЛЬЦА цели до сервера добавит сам сервер
## (Car.net_wire_lag) — этой половины клиенту неоткуда знать.
static func net_view_lag() -> float:
	return maxf(0.0, net_buf_delay + Net.half_rtt(1) - net_view_lead)


## Полная отмотка ДЛЯ КОНКРЕТНОЙ ЦЕЛИ: то, что доложил стрелявший, плюс
## полпути владельца цели до сервера. base == 0 (бот, оффлайн) — не
## отматываем вовсе: их картина и есть правда.
static func aim_lag(base: float, target: Car) -> float:
	if base <= 0.0 or target == null:
		return 0.0
	return base + target.net_wire_lag
...
## Без трассы или без стен (футбол) — как есть. Не путать с
...
func _follow_buffered(delta: float) -> void:
	...
	# УПРЕЖДЕНИЕ: сдвигаем цель вперёд по ДУГЕ — на скорости из записи и с
	# её же темпом разворота. Прямая экстраполяция в повороте уводит
	# соперника наружу, в ограждение; дуга держит его на своей траектории.
	# По высоте не упреждаем вовсе: на трамплине это выглядело бы
	# «подпрыгнул раньше времени».
	if net_view_lead > 0.0:
		var k_lead := 1.0 - pow(0.80, delta * 60.0)
		var vh := _play_vel
		vh.y = 0.0
		_lead_vel = _lead_vel.lerp(vh, k_lead)
		if _buf.size() >= 2:
			var span_r := maxf((_buf[1]["t"] as float)
					- (_buf[0]["t"] as float), 0.001)
			var w := angle_difference(_yaw_of(_buf[0]["rot"]),
					_yaw_of(_buf[1]["rot"])) / span_r
			_lead_yaw = lerpf(_lead_yaw, clampf(w, -3.0, 3.0), k_lead)
		var ang := clampf(_lead_yaw * net_view_lead, -1.0, 1.0)
		var span_t := net_view_lead
		if absf(_lead_yaw) > 0.05:
...
func _drive(
	...
	if on_ground:
		# Рыхлый песок не только режет тягу и потолок — он активно ТОРМОЗИТ
		# уже разогнавшуюся машину до песчаного потолка (01.09: «песок должен
		# тормозить сильнее»: один множитель 0.40 скорость ВЫШЕ потолка не
		# гасил вовсе — тяга просто отключалась, и машина неслась по песку
		# накатом, теряя ход лишь трением).
		if _on_sand:
			var sand_h := linear_velocity
			sand_h.y = 0.0
			if sand_h.length() > eff_max:
				apply_central_force(
						-sand_h.normalized() * brake_power * 1.3 * mass * 0.1)
...
func _wall_slide(delta: float) -> void:
	# На трассе без ограждений (футбол, стенды) вести не вдоль чего.
	# (До 14.09 такой была и песчаная: съезд на песок был легален, его
	# наказывал сам песок — см. _on_sand; теперь и там отбойники.)
	...
	# ГИСТЕРЕЗИС порога (08.09). С одним порогом состояние «касается»
	# трещало: депенетрация ставит кузов ровно на грань, и каждые
	# 8-16 кадров касание на кадр пропадало. В такой кадр ведение молчит,
	# шины успевают довернуть скорость в стену, а на следующем кадре
	# ведение возвращает её на касательную разом — рывок 45 м/с² при
	# ровных 16 (стенд TestWallLeave). Отпускаем, только отойдя на 0.35 м.
	var touching := dist + reach > wall_face - (0.35 if _wall_touch else 0.05) \
			and dist < wall_face + 0.3
	_wall_touch = touching
	_wall_near = touching
	...
	if touching or _wall_align_time > 0.0:
		...
		# ...НО не режем уход, который объясняет САМ НОС машины. Едешь прямо
		# вдоль загнутого борта — трасса уходит вбок, и машина честно
		# отделяется от стены со скоростью v·sin(угол к касательной): на
		# 25 м/с и 10° это 4.3 м/с, вчетверо больше капа. Кап держал её
		# у борта, а через 0.35 с окна отпускал — «в момент отрыва машину
		# подёргивает в сторону борта» (жалоба 08.09). Отброс решателя от
		# ребра, ради которого кап и стоит, носом не объясняется: он всегда
		# ВБОК от того, куда машина смотрит.
		var nose_out := fwd.dot(n) * h_now.length()
		out_cap = maxf(out_cap, -nose_out)
		if out_now < -out_cap:
	...
	if steering_away or _slip_time > 0.0:
		return
	# Доворот — только пока кузов ДЕЙСТВИТЕЛЬНО у грани. Раньше он жил ещё
	# 0.35 с после схода (_wall_align_time), и оторвавшуюся машину стена
	# продолжала «вести» вслед за своим изгибом, обнуляя рысканье, — а по
	# истечении окна руль возвращался разом: тот самый рывок в сторону
	# борта при отрыве (жалоба 08.09). Гасить закрутку от касания углом
	# после схода больше незачем: кузов со стеной в решателе не
	# сталкивается вовсе (исключение выше), закручивать нечему.
	if not touching:
		return
...
func _use_magnet() -> void:
	...
	for node in get_tree().get_nodes_in_group("cars"):
		...
		var dist := global_position.distance_to(
				other.past_position(aim_lag(net_shot_lag(), other)))
...
## хорде — «мимо трассы», и на песчаной трассе (до 14.09 — без ограждений)
## это заканчивалось полётом в пески. У края рывок остаётся, но идёт ВДОЛЬ
...
func _laser_sweep(fwd: Vector3) -> void:
	# Полуширина коридора поражения. Было 1.6 м при полуширине кузова
	# 0.85 и толщине луча 0.11: машина гибла, когда луч проходил в
	# полуметре ОТ неё — «лазер его не касается, но уничтожает» (жалоба
	...
	for node in get_tree().get_nodes_in_group("cars"):
		...
		if other == self or not other.alive or other.is_ghost():
			continue
		# Кузов — ОТРЕЗОК, а не точка (как у снаряда): машина 3.2 м длиной,
		# и по одному центру луч, прошивший нос или корму, не считался
		# попаданием. Три пробы с узким коридором честнее одной с широким.
		var center := other.past_position(aim_lag(_laser_lag, other))
		var body_f := other.true_forward()
		var hit := false
...
## Сам призрак живёт на слое 8: машины (маска 0b111) его не видят, а зоны
## БОНУСОВ — бокс с оружием и плита ускорения (маска 0b1100) — видят
## (03.09: с нулевым слоем призрак 2 с после взрыва проезжал сквозь
## боксы и плиты, «проезжаешь сквозь бонус, не собирая»). Мины, масло и
## снаряды ловят только слой 4 — призрак для них по-прежнему неуязвим.
