...
## Поперечные смещения мин (м) по ступеням — ОДНА таблица для сервера и
## оффлайна (Car.use_weapon) и для клиентской копии (Main._spawn_weapon_visual):
## разойдись они, игрок видел бы мину не там, где она лежит на сервере.
## II — две под левое и правое колесо; III — три в ряд, «немного подальше
## друг от друга» (просьба 10.09): 1.9 м между соседними вместо 1.0 —
## корпуса (r 0.72) раньше почти слипались в одну кучу, теперь ряд
## перекрывает ~4.6 м полотна.
static func mine_offsets(step: int) -> Array[float]:
	if step >= 3:
		return [-1.9, 0.0, 1.9]
	if step >= 2:
		return [-0.8, 0.8]
