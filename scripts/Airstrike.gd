...
## Тени вдоль оси перед лидером, м. Шесть отметок — потолок ступени III
## (спецификация игрока 09.09: I — 4 ракеты, II — 5, III — 6); лишние не
## используются, `want` берёт столько, сколько ракет у стрелка.
const AHEAD := [10.0, 16.0, 22.0, 28.0, 34.0, 40.0]
...
## Ступени авиаудара (магазин, 08.09; спецификация игрока 09.09):
## rockets — сколько ракет (без ступеней две, I — четыре, II — пять,
## III — шесть, потолок AHEAD.size()); lead — со II
...
func _ready() -> void:
	...
	if track == null:
		...
		for ahead: float in [2.0, 8.0, 14.0, 20.0, 26.0, 32.0].slice(
				0, clampi(rockets, 1, 6)):
