...
var _tick_base := 0
# Метки идут по кругу через STAMP_WRAP тиков (~4.9 ч): переполнение видно
# клиенту как «метка прыгнула назад» — буфер очищается один раз, кадр
# незаметного шва. Метки СЕРВЕРА (боты) сдвинуты на STAMP_SERVER_BASE:
# диапазоны сервера и владельца не пересекаются, и смена автора
# состояния (бот <-> игрок) всегда читается как скачок, а не как
# продолжение записи (иначе при близких счётчиках сцен шкала уезжала на
# секунды вперёд и марионетка ползла).
const STAMP_WRAP := 1 << 20
const STAMP_SERVER_BASE := 1 << 21
...
func _setup_hud() -> void:
	...
	if TouchControls.wanted():
		...
		_weapon_name.position = Vector2(0, 190)
		_weapon_name.size = Vector2(228, 22)
...
func _pack_state() -> Array:
	...
	for ci in n:
		...
		# одному на тик.
		var stamp := float(STAMP_SERVER_BASE
				+ (Engine.get_physics_frames() - _tick_base) % STAMP_WRAP)
...
(у сервера %d, у тебя %d). Обновите игру." % [Net.PROTOCOL, proto])
...
func _rx_state(xf: PackedFloat32Array, flags: PackedByteArray,
	...
	if _last_state_time > 0.0:
		...
		if Net.is_client():
			# Формула буфера — прежняя, по сырому интервалу прихода: попытка
			# вычитать закрытые историей дыры и собственные фризы кадра
			# (02.09) опустила буфер ниже нужного, и соперники дёргались.
