...
func _physics_process(_d: float) -> void:
	...
	match _frame:
		...
		310:
			...
			# 11.09: волна больше НЕ гаснет о щит — она вообще гаснет только
			# от времени (просьба игрока «гаснет от времени, а не от первого
			# столкновения»), поэтому летит дальше, а щит просто не пустил
			# эффект (проверка выше).
			_check("глушилка: волна летит дальше, а не гаснет о щит",
					not _nodes(ScrambleWave).is_empty(),
					"волн %d" % _nodes(ScrambleWave).size())
