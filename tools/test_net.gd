...
##   4) метки над всеми соперниками видны (с 01.09 — и над ботами: бот не
##      должен отличаться от живого игрока);
...
##
## Живой игрок от бота отличается теперь ТОЛЬКО маской _slot_taken (по ней
## же _rx_state берёт метку часов сервера у слота без игрока).
##
...
func _ready() -> void:
	# Headless без ограничения крутит _process тысячи раз в секунду и
	# выедает ядро busy-loop'ом; вместе с локальным сервером (второй такой
	# же процесс) стенд ловил ПАУЗЫ ПЛАНИРОВЩИКА ОС до 1.8 с, которые
	# выглядели как «фризы клиента посреди гонки» — в настоящей игре с
	# vsync такого нет. Кап делает стенд честным.
	Engine.max_fps = 120
...
func _sample_smoothness(delta: float) -> void:
	# Бот-марионетка: чужой слот без живого игрока. Раньше это читалось по
	# видимой метке соперника, но с 01.09 стрелки видны и над ботами
	# (бот неотличим от игрока) — берём честную маску занятых слотов.
	var idx := -1
	var rv := -1
	for i in _main._cars.size():
		if i == Net.my_slot:
			continue
		var taken: bool = i < _main._slot_taken.size() \
				and _main._slot_taken[i]
		if taken and rv < 0:
			rv = i
...
func _sample_one(car: Car, delta: float, steps: Array[float],
	...
	# contact_velocity, а не _snap_vel: клиентская марионетка с 27.08 едет
	# по ЗАПИСИ снимков с отставанием, и честная скорость — скорость
	# воспроизводимого куска (сверка с последним пакетом штрафовала бы
	# буфер за само его устройство — та самая ошибка замера 24.08).
	var vel := car.contact_velocity()
	vel.y = 0.0
