...
## на 20 м/с. Стена должна НАПРАВИТЬ её вдоль трассы, не тормозя в ноль.
...
## 07.09: вариант со скрежетом вдоль стены (нижняя граница 8) отменён по
## просьбе игрока — ход вдоль ограждения не теряется, только искры; от
## «езды без руля» лечит доворот с мёртвой зоной (Car.WALL_ALIGN_FREE).
