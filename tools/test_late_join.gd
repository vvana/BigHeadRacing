...
##   поднимется всегда;
## - 31.08: JOIN_LATE_MAX отменён — подсадки в идущий заезд нет ВООБЩЕ.
##   Опоздавшим считается всякий, кто пришёл после старта, поэтому ждать
##   перед запуском стенда больше не нужно.
##
...
## дождаться отсчёта «3-2-1-GO» на нём, и только потом:
...
func _physics_process(delta: float) -> void:
	...
	# положен — их заезд уже начался, а начавшийся новых не берёт).
	if Net.my_slot >= 0:
		_finish()
