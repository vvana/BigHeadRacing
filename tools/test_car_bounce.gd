...
func _physics_process(_delta: float) -> void:
	...
	if local == 0:
		...
		if _phase == 3:
			...
			# толчок есть, борта расталкиваются (>= 2.0 м; перекрытие
			# корпусов заканчивается на 1.7 — порог с запасом. Было 2.2,
			# но на ПЛОСКОЙ трассе без попутного уклона факт 2.18).
			...
			print("CARBOUNCE TEST: %s (угол >= 20, центр < 8, толчок >= 1.5, гэп >= 2.0)"
