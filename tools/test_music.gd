extends Node
## Автотест музыки (03.09). Проверяет, что все пять файлов на месте и
...
func _ready() -> void:
	...
	# вплотную и весь список успел побывать (гоночных четыре).
	var seen := {}
	...
	for i in 6:
		...
		prev = Music._current
	_ok(seen.size() == Music.RACE.size(), "за шесть заездов слышен весь список")
