...
func _run() -> void:
	...
	_ok(int(gs.stats.wins) == 1 and int(gs.stats.podiums) == 2,
			"побед 1, подиумов 2")
	_ok(int(gs.stats.kills) == 3 and int(gs.stats.best_place) == 1,
			"уничтожено 3, лучшее место 1")
